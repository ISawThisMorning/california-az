    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: 900, margin: '0 auto', padding: '24px 16px' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          {listings.map(l => (
            <div key={l.id} style={{ border: '1px solid #e5e7eb', borderRadius: 12, padding: 20, display: 'flex', gap: 16, background: l.starred ? '#fffbeb' : '#fff' }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, background: { airbnb: '#ff385c', vrbo: '#3D6EF6', craigslist: '#7c3aed' }[l.source] || '#999', color: '#fff', padding: '2px 8px', borderRadius: 4, textTransform: 'uppercase' }}>{l.source}</span>
