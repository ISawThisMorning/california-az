...
const SEARCH_CONFIGS = {
  ...
  marina: {
    craigslist: {
      searchUrls: ['https://losangeles.craigslist.org/search/hhh?query=marina+del+rey+furnished+rental+monthly'],
      maxItems: 50,
    },
    zillow: {
      searchUrls: [{ url: 'https://www.zillow.com/marina-del-rey-ca/rentals/?searchQueryState=%7B%22isMapVisible%22%3Atrue%2C%22mapBounds%22%3A%7B%22north%22%3A34.0%2C%22south%22%3A33.97%2C%22east%22%3A-118.43%2C%22west%22%3A-118.47%7D%2C%22filterState%22%3A%7B%22fr%22%3A%7B%22value%22%3Atrue%7D%2C%22fsba%22%3A%7B%22value%22%3Afalse%7D%2C%22fsbo%22%3A%7B%22value%22%3Afalse%7D%7D%2C%22isListVisible%22%3Atrue%7D' }],
    },
  },
}
...
function normalizeZillow(item, area) {
  ...
  // Extract lat/lng from id field if it looks like "33.98875--118.45342"
  let latitude = item.latitude || item.lat || null
  let longitude = item.longitude || item.lng || null
  if (!latitude && item.id && String(item.id).includes('--')) {
