...
const SEARCH_CONFIGS = {
  tahoma: {
    airbnb: { locationQuery: 'Tahoma, CA', checkIn: getDateString(0), checkOut: getDateString(90), minBedrooms: 2, maxResults: 20 },
    craigslist: { site: 'reno', query: 'tahoma tahoe west shore house rental', category: 'apa' },
  },
  venice: {
    airbnb: { locationQuery: 'Venice Beach, Los Angeles, CA', checkIn: getDateString(0), checkOut: getDateString(90), minBedrooms: 2, maxResults: 20 },
...
function normalizeAirbnb(item, area) {
  ...
  return {
    source: 'airbnb', external_id: String(item.id || ''), url: item.url || `https://airbnb.com/rooms/${item.id}`,
    area, title: item.name || item.title, description: item.description,
    bedrooms: item.bedrooms, bathrooms: item.bathrooms, max_guests: item.personCapacity,
    property_type: item.propertyType, price_per_night: nightlyPrice,
    price_per_month: nightlyPrice ? Math.round(nightlyPrice * 30 * 0.85) : null,
    price_raw: item.price?.label || String(nightlyPrice),
    lat: item.lat, lng: item.lng, city: item.city, raw: item,
  }
}
...
function normalizeVrbo(item, area) {
  ...
  return {
    source: 'vrbo', external_id: String(item.id || ''), url: item.detailPageUrl || item.url,
    area, title: item.headline || item.name, description: item.description,
    bedrooms: item.bedrooms, bathrooms: item.bathrooms, max_guests: item.sleeps,
    property_type: item.propertyType, price_per_night: nightlyPrice,
    price_per_month: nightlyPrice ? Math.round(nightlyPrice * 30 * 0.85) : null,
    price_raw: String(nightlyPrice), lat: item.lat, lng: item.lng, city: item.city, raw: item,
  }
}

function normalizeCraigslist(item, area) {
  return {
    source: 'craigslist', external_id: String(item.id || item.postId || ''),
    url: item.url || item.link, area, title: item.title, description: item.body,
    bedrooms: item.bedrooms, bathrooms: item.bathrooms,
    price_per_month: item.price ? parseInt(item.price.replace(/\D/g, '')) : null,
    price_raw: item.price, city: item.location, raw: item,
  }
}
